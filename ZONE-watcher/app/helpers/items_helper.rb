module ItemsHelper
  def getButtonForFilter(filter)
    ...
    if(filter.prop.starts_with? @OPEN_CALAIS_URI)
      ...
      res= link_to filter.value, itemURI, :class =>  itemClassFilter, title: filter.prop[@OPEN_CALAIS_URI.length,filter.prop.length]
      ...
      if(filter.value.rindex('/') == nil)
        res=link_to filter.value, itemURI, :class => itemClassFilter, title: filter.prop[@WIKI_META_URI.length,filter.prop.length]

      else
        res=link_to filter.value[filter.value.rindex('/')+1, filter.value.length], itemURI, :class => itemClassFilter, title: filter.prop[@WIKI_META_URI.length,filter.prop.length]
      end
